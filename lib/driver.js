...
class AndroidDriver extends BaseDriver {
  ...
  async startAndroidSession() {
    ...
    this.defaultIME = await helpers.initDevice(this.adb, this.opts, this.caps.forceInstall ? this : null);

    //logcat deviceLogPath
    if (this.opts.deviceLogPath) {
      log.debug('Testwa device log initiating!');
      this.logcatProcess = testwa.startLogcat(this.adb, this.opts, this.sessionId);
    }
  ...
  async initAUT() {
    ...
    await helpers.installApkRemotely(this.adb, this.opts, this.caps.forceInstall ? this : null);
    await this.grantPermissions();
