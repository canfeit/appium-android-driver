...
class AndroidDriver extends BaseDriver {
  ...
  async createSession (caps) {
    ...
    try {
      ...
      if (this.opts.app) {
        ...
        await this.checkAppPresent(); //check app on pc
        ...
        await this.checkPackagePresent(); // check package on device
      }
      ...
      if (!_.isUndefined(this.opts.networkSpeed)) {
        if (!this.isEmulator()) {
  ...
  async startAndroidSession () {
    ...
    // start UiAutomator (before push apps... testwa)
    ...
    // testwa: Manufacturer and language get this first to enable force install
    this.caps.deviceManufacturer = await this.adb.getManufacturer();
    this.caps.language = await this.adb.getDeviceLanguage();

    // set up the device to run on (real or emulator, etc)
    this.defaultIME = await helpers.initDevice(this.adb, this.opts, this.caps.autoConfirm ? this : null);

    //logcat deviceLogPath
  ...
  async initAUT () {
    ...
    // catch error for now testwa
    try {
      await helpers.installApkRemotely(this.adb, this.opts, this.caps.autoConfirm ? this : null);
    } catch (e) {
      logger.error("Installing remote APK failed, skip it for now!");
    }
    await this.grantPermissions();
