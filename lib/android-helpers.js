...
helpers.prepareEmulator = async function (adb, opts) {
  let {
    avd, avdArgs, language, locale, avdLaunchTimeout,
    avdReadyTimeout
  } = opts;
  if (!avd) {
...
helpers.installApkRemotely = async function (adb, opts, androidDriver) {
...
helpers.initUnicodeKeyboard = async function (adb, androidDriver) {
  logger.debug('Enabling Unicode keyboard support');
  logger.debug("Pushing unicode ime to device...");
  await adb.install(androidDriver, unicodeIMEPath, false);
...
helpers.pushUnlock = async function (adb, androidDriver) {
  logger.debug("Pushing unlock helper app to device...");
  try {
    await adb.install(androidDriver, unlockApkPath, false);
  } catch (err) {
    logger.warn(`Ignored error while installing Appium Unlock: "${err.message}". `);
  }
};
