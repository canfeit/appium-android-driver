...
helpers.reinstallRemoteApk = async function (androidDriver, adb, localApkPath, pkg,
  ...
  await retry(tries, async () => {
    ...
    try {
      logger.debug('install apk ... testwa');
      await adb.installFromDevicePath(androidDriver, remotePath, {timeout: androidInstallTimeout});
    } catch (e) {
...
helpers.installApkRemotely = async function (androidDriver, adb, opts) {
  ...
  if (installed && remoteApkExists && fastReset) {
    ...
    await helpers.reinstallRemoteApk(androidDriver, adb, app, appPackage, remotePath, androidInstallTimeout);
  }
};
...
helpers.initUnicodeKeyboard = async function (adb) {
  ...
  await adb.install(null,unicodeIMEPath, false);
...
helpers.pushSettingsApp = async function (adb, androidDriver) {
  logger.debug("Pushing settings apk to device...");
  try {
    if (!androidDriver) {
      await adb.installOrUpgrade(null, settingsApkPath, SETTINGS_HELPER_PKG_ID);
    } else {
      // force install
      await adb.installOrUpgrade(androidDriver, settingsApkPath, SETTINGS_HELPER_PKG_ID);
    }
  } catch (err) {
  ...
  // await Promise.all([adb.install(settingsApkPath, true), helpers.forceInstall(androidDriver, "确定","安装")], helpers.forceInstall(androidDriver, '//android.widget.Button[@text="OK"]','//android.widget.Button[@text="Install"]'));
  //确定》安装 or OK >> Install
  // stra: -android uiautomator   selector: new UiSelector().text("Add Contact")
  // str: xpath   selector:　//android.widget.Button[@text="Add Contact"]
};

helpers.forceInstall = async function (androidDriver, locator1, locator2) {
  logger.debug("Forcing install...");
  try {
    let element = await androidDriver.findElOrEls("xpath", locator1, false);
    logger.debug("Find element "+ element);
    await androidDriver.click(element.ELEMENT);
    element = await androidDriver.findElOrEls("xpath", locator2, false);
    logger.debug("Find element "+ element);
    await androidDriver.click(element.ELEMENT);
  } catch (err) {
    // logger.debug(err);
...
helpers.pushUnlock = async function (adb) {
  ...
  await adb.install(null, unlockApkPath, false);
};
...
helpers.initDevice = async function (adb, opts, androidDriver) {
  ...
  if (!opts.avd) {
    await helpers.pushSettingsApp(adb, androidDriver);
  }
